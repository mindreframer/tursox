...
impl Connection {
    ...
    /// Enables or disables Turso's native runtime extension loader.
    pub fn set_load_extension_enabled(&self, enabled: bool) -> Result<()> {
        self.get_inner_connection()?.set_load_extension_enabled(enabled);
        Ok(())
    }
